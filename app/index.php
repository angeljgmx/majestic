...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------//
    ...
    if ((isset($_GET['error'])) && ($_GET['error'] == 'bad_password')){
        $titulo = "Email no registrado";
        $mensaje = "La contrase&ntilde;a que ha ingresado no es correcta<br />Verifique los datos ingresados y pruebe nuevamente<br /><br />Para cualquier inconveniente comuniquese por nuestros n&uacute;mero de contacto ".$_SESSION['cont_tlfn']; 
            ...
            if ($nuser > 0){
                ...
                if ($datos_user->user_estd != 0){
                    ...
                    if (password_verify($logn_pass, $datos_user->user_pass)) {
                        ...
                        echo "<script type=\"text/javascript\">window.location=\"".$path."app/index.php?error=bad_password\"</script> \n";   
                    }
                }
                else {
                ...
                $titulo = "Email no registrado";
                $mensaje = "La direcci&oacute;n de correo electr&oacute;nico que ha ingresado<br /> no se encuentra registrada en nuestro sistema <br />Verifique los datos ingresados y pruebe nuevamente<br /><br />Para cualquier inconveniente comuniquese por nuestros n&uacute;mero de contacto ".$_SESSION['cont_tlfn']; 
                ModalMensaje($path, $titulo, $mensaje);
            }
        }
        else {
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------//
    ...
    if ((isset($_SESSION['activo'])) && ($_SESSION['activo'] == FALSE)){
        ...
        $mensaje = "Su usuario se encuentra inhabilitado, recuerde cancelar su membresia <br /><br />Para cualquier inconveniente comuniquese por nuestros n&uacute;mero de contacto ".$_SESSION['cont_tlfn']; 
        ModalMensaje($path, $titulo, $mensaje);
    ...
    <section class="login_index"> 
        <div class="container">
            <div class="row text-center pt-80">
                <img width="360" src="<?php echo $path."uploads/imagenes/logo/".$datos_pref->pref_logh; ?>" alt="<?php echo $_SESSION['cont_nomb']; ?>">
            </div>
            <div class="row">
                <div class="col-md-4 col-md-offset-4 mb-200">
                    <form class ="login mt-80 mb-40" action="<?php echo $path."app/index.php"; ?>" method="post">
                        <div class="form-group mb-20">
                            ...
                            <input id="login_email" name="user_mail" placeholder="Ingrese su direcci&oacute;n de correo electr&oacute;nico" type="email" class="form-control form-value" required="required">
                        </div>
            ...
            <div class="row footer_index">
                <div class="col-md-4">
                    <address>
                        <p><span>Llamanos</span> <?php echo $datos_cont->cont_tlfn; ?></p>
                    </address>
                </div>
                <div class="col-md-4">
                    <address>
                        <p><span>Estamos ubicados en</span> <?php echo $datos_cont->cont_dirc; ?></p>
                    </address>
                </div>
                <div class="col-md-4">
                    <address>
                        <p><span>Email</span> <a href="mailto:<?php echo $datos_cont->cont_dirc; ?>"><?php echo $datos_cont->cont_mail; ?></a></p>
                    </address>
                </div>
            </div>
...
<div>
    <?php
    echo $fecha_actual = date("d-m-Y H:i:s")
    ?>
</div>
