    ...
    for ($i=0; $i<$numDirs; $i++) {
      ...
      if (!in_array($this->scriptArray[$i], $this->removeDirs)) {
      	...
      	elseif (!in_array($this->scriptArray[$i], $this->removeDirs) || $showLink=='yes') 
					$breadcrumb[] = '<li><a href="' . $dir . $rootFileName . '"' . $class . $target . ' title="' . $dirTxtName . '">' . $dirName .$this->symbol . '</a></li>';
...
//                                           $fileName.'</span>';
      ...
      if ($this->linkFile==TRUE){
        $fileName = '<li><a href="'.$originalFileName.'">'.$fileName.'</a></li>';
      } else {
    ...
    if ($numDirs>0) return @implode($breadcrumb).$fileName;
